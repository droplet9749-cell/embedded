...
// 배터리 사양 및 환경정보 입력
printf("평균 전류 I_load [A] (부하측) : ");      if (scanf("%1f", &I_load)!=1) return 1;
printf("평균 속도 vel [km/h] : ");              if (scanf("%1f", &vel)!=1) return 1;
printf("예비율 R [20% = 0.2] : ");              if (scanf("%1f", &R)!=1) return 1;

printf("공칭전압 V [V] : ");                    if(scanf("%1f", &V)!=1) return 1;
printf("용량 C [ah] : ");                      if(scanf("%1f", &C)!=1) return 1;
printf("시스템효율 eta [90% = 0.9] : ");         if(scanf("%1f", &eta)!=1) return 1;
